...
def create_order_detail(od: schemas.OrderDetailCreate, db: Session = Depends(get_db)):
    return order_details.create(db=db, od=od)
...
@app.get("/order_details/{od_id}", response_model=schemas.OrderDetail, tags=["Order Details"])
def read_one_order_detail(od_id: int, db: Session = Depends(get_db)):
    od = order_details.read_one(db, od_id=od_id)
    if od is None:
        raise HTTPException(status_code=404, detail="Order detail not found")
    return od


@app.put("/order_details/{od_id}", response_model=schemas.OrderDetail, tags=["Order Details"])
def update_order_detail(od_id: int, od: schemas.OrderDetailUpdate, db: Session = Depends(get_db)):
    od_db = order_details.read_one(db, od_id=od_id)
    if od_db is None:
        raise HTTPException(status_code=404, detail="Order detail not found")
    return order_details.update(db=db, od=od, od_id=od_id)


@app.delete("/order_details/{od_id}", tags=["Order Details"])
def delete_order_detail(od_id: int, db: Session = Depends(get_db)):
    od = order_details.read_one(db, od_id=od_id)
    if od is None:
        raise HTTPException(status_code=404, detail="Order detail not found")
    return order_details.delete(db=db, od_id=od_id)
